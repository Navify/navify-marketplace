...
function Invoke-Navify {
    ...
    $navifyArgs = @()
    if ($BypassAdmin) {
        $navifyArgs += "--bypass-admin"
    }
    $navifyArgs += $Arguments
    
    & navify $navifyArgs
    return $LASTEXITCODE
}

function Invoke-NavifyWithOutput {
    param (
        [Parameter(Mandatory = $true, Position = 0, ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )
    ...
    $output = (& navify $navifyArgs 2>&1 | Out-String).Trim()
    return @{
        Output = $output
...
if (-not (Get-Command -Name 'navify' -ErrorAction 'SilentlyContinue')) {
    Write-Host -Object 'Navify not found.' -ForegroundColor 'Yellow'
    Write-Host -Object 'Installing it for you...' -ForegroundColor 'Cyan'
    $Parameters = @{
        Uri             = 'https://raw.githubusercontent.com/navify/cli/main/install.ps1'
...
try {
    $result = Invoke-NavifyWithOutput "path" "userdata"
    if ($result.ExitCode -ne 0) {
        Write-Host -Object "Error from Navify:" -ForegroundColor 'Red'
        Write-Host -Object $result.Output -ForegroundColor 'Red'
        return
    }
    $spiceUserDataPath = $result.Output
} catch {
    Write-Host -Object "Error running Navify:" -ForegroundColor 'Red'
...
if (-not (Test-Path -Path $spiceUserDataPath -PathType 'Container' -ErrorAction 'SilentlyContinue')) {
    $spiceUserDataPath = "$env:APPDATA\navify"
}
$marketAppPath = "$spiceUserDataPath\CustomApps\marketplace"
$marketThemePath = "$spiceUserDataPath\Themes\marketplace"

$isThemeInstalled = $(
    Invoke-Navify "path" "-s" | Out-Null
    -not $LASTEXITCODE
)
$currentTheme = (Invoke-NavifyWithOutput "config" "current_theme").Output
$setTheme = $true
...
try {
    $result = Invoke-NavifyWithOutput "path" "userdata"
    if ($result.ExitCode -ne 0) {
        Write-Host -Object "Error: Failed to get Navify path. Details:" -ForegroundColor 'Red'
        Write-Host -Object $result.Output -ForegroundColor 'Red'
        return
    }
...
$Parameters = @{
  Uri             = 'https://github.com/navify/marketplace/releases/latest/download/marketplace.zip'
  UseBasicParsing = $true
...
Invoke-Navify "config" "custom_apps" "navify-marketplace-" "-q"
Invoke-Navify "config" "custom_apps" "marketplace"
Invoke-Navify "config" "inject_css" "0" "replace_colors" "0" "overwrite_assets" "0" "inject_theme_js" "0"

Write-Host -Object 'Downloading placeholder theme...' -ForegroundColor 'Cyan'
$Parameters = @{
  Uri             = 'https://raw.githubusercontent.com/navify/marketplace/main/resources/color.ini'
  UseBasicParsing = $true
...
if ($setTheme) {
    Invoke-Navify "config" "current_theme" "marketplace"
}
Invoke-Navify "backup"
Invoke-Navify "apply"
