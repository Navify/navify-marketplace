...
> {
  ...
  async loadPage(queue: never[]) {
    ...
    switch (activeTab) {
      case "Extensions": {
        const pageOfRepos = await getTaggedRepos("navify-extensions", this.requestPage, this.BLACKLIST, this.CONFIG.visual.showArchived);
        const extensions: CardItem[] = [];
        if (!this.requestPage) extensions.push(...getLocalCatalog("extension"));
        const extensionGroups = await Promise.all(
          pageOfRepos.items.map(async (repo) => {
            const items = await fetchExtensionManifest(
              repo.contents_url,
              repo.default_branch,
              repo.stargazers_count,
              this.CONFIG.visual.hideInstalled
            );
            return (items || []).map((extension) => ({
              ...extension,
        ...
        extensions.push(...extensionGroups.flat());
      ...
      case "Installed": {
        ...
        for (const configured of getConfiguredCatalog()) {
      ...
      case "Themes": {
        ...
        if (!this.requestPage) {
          for (const localTheme of getLocalCatalog("theme")) {
            this.appendCard(localTheme, "theme", activeTab);
          }
          this.setState({ cards: this.cardList });
        }

        const pageOfRepos = await getTaggedRepos("navify-themes", this.requestPage, this.BLACKLIST, this.CONFIG.visual.showArchived);
        const themeGroups = await Promise.all(
          pageOfRepos.items.map(async (repo) => {
            const items = await fetchThemeManifest(repo.contents_url, repo.default_branch, repo.stargazers_count);
      ...
      case "Apps": {
        const pageOfRepos = await getTaggedRepos("navify-apps", this.requestPage, this.BLACKLIST, this.CONFIG.visual.showArchived);
        const apps: CardItem[] = [];
        if (!this.requestPage) apps.push(...getLocalCatalog("app"));

        const appGroups = await Promise.all(
          pageOfRepos.items.map(async (repo) => {
            const items = await fetchAppManifest(repo.contents_url, repo.default_branch, repo.stargazers_count);
  ...
  updateColourSchemes(schemes: SchemeIni, activeScheme: string | null) {
    ...
    if (activeScheme) (Navify.Config as { [key: string]: unknown }).color_scheme = activeScheme;
