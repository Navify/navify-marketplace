...
const BackupModal = () => {
  ...
  const exportSettings = async () => {
    ...
    try {
      ...
      Navify.showNotification(t("backupModal.settingsSaved"));
    } catch (error: unknown) {
      if (!isAbortError(error)) {
        console.error("Failed to save file, copying to clipboard instead:", error);
        Navify.Platform.ClipboardAPI.copy(JSON.stringify(settings));
        Navify.showNotification(t("backupModal.settingsCopied"));
      }
    }
  ...
  const importSettings = async (settingsString: string) => {
    ...
    if (!settingsString) {
      Navify.showNotification(t("backupModal.noDataPasted"));
      return;
    }
    ...
    try {
      ...
      Navify.showNotification(t("backupModal.invalidJSON"));
      return;
    }
    ...
    try {
      ...
      Navify.showNotification(error instanceof Error ? error.message : t("backupModal.invalidJSON"), true);
    }
  };
