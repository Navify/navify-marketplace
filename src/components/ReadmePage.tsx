...
> {
  ...
  getReadmeHTML = async () => {
    return fetch(this.props.data.readmeURL)
      .then((res) => {
        if (!res.ok) throw Navify.showNotification(`${this.props.t("readmePage.errorLoading")} (HTTP ${res.status})`, true);
        return res.text();
      })
      .then((readmeText) => getMarkdownHTML(readmeText, this.props.data.user, this.props.data.repo))
      .then((html) => {
        if (!html) Navify.Platform.History.goBack();
        return html;
      })
      .catch((err) => {
        console.error(err);
  ...
  componentDidUpdate() {
    ...
    // so I turn it into https://raw.githubusercontent.com/theRealPadster/navify-hide-podcasts/main/screenshot.png
