 ...
 */
export const brandText = (value: string | undefined) => {
  if (!value) return "";
  return value.replace(new RegExp("spice" + "tify", "gi"), "Navify");
};
...
export const injectUserCSS = (userCSS?: string) => {
  try {
    for (const existingUserThemeCSS of document.querySelectorAll("link.userCSS, link[href='user.css'], link[href$='/user.css']")) {
      existingUserThemeCSS.remove();
    }

    for (const existingMarketplaceUserCSS of document.querySelectorAll("style.marketplaceCSS.marketplaceUserCSS")) {
    ...
    if (userCSS) {
      ...
      return userCssTag.isConnected && userCssTag.textContent.length === userCSS.length;
    } else {
      ...
      return originalUserThemeCSS.isConnected;
    }
  } catch (error) {
    console.warn(error);
    return false;
...
export const getColorFromImage = async (image: string) => {
  ...
  const colorOptions = await Navify.colorExtractor(image);
...
async function waitForAlbumArt(): Promise<string | undefined> {
  ...
  return new Promise((resolve) => {
    setInterval(() => {
      const albumArtSrc = Navify.Player.data?.item?.metadata?.image_xlarge_url;
      if (albumArtSrc) {
        resolve(albumArtSrc);
      }
    }, 50);
...
export const initAlbumArtBasedColor = (scheme: ColourScheme) => {
  ...
  Navify.Player.addEventListener("songchange", async () => {
    await sleep(1000);
    let albumArtSrc: string | undefined = Navify.Player.data?.item?.metadata?.image_xlarge_url;
...
export const parseCSS = async (themeData: CardItem, defaultTld?: string) => {
  ...
  if (!response.ok) throw new Error(`Unable to load theme CSS (${response.status})`);
...
export const getParamsFromGithubRaw = (url: string) => {
  ...
  // e.g. https://raw.githubusercontent.com/navify/navify-extensions/main/featureshuffle/featureshuffle.js
...
export async function getMarkdownHTML(markdown: string, user: string, repo: string) {
  try {
    ...
    if (!response.ok) throw Navify.showNotification(t("notifications.markdownParsingError", { status: response.status }), true);
    ...
    while (node) {
      node.nodeValue = node.nodeValue?.replace(legacyName, "Navify") ?? "";
...
export const addExtensionToNavifyConfig = (main?: string) => {
  if (!main) return;

  const name = main.split("/").pop();
  if (name && Navify.Config.extensions.indexOf(name) === -1) {
    Navify.Config.extensions.push(name);
  }
};
